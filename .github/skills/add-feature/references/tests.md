...
Stack: xUnit + Shouldly + NSubstitute (unit), FluentValidation.TestHelper (validators), Aspire Testing through the AppHost (integration). Every new use case gets all three.
...
`tests/IntegrationTests/{Feature}/{Feature}Tests.cs` (extend the existing file if present). Inherit `BaseIntegrationTest(appHost)` — it runs the real API through the Aspire AppHost with SQL Server and provides `HttpClient`, `RegisterAndLoginAsync()`, and `Authenticate(token)`. Tests go through real HTTP, never call handlers directly.
...
Integration tests need a container runtime for the Aspire AppHost SQL Server resource. Architecture tests will fail the build if a layer dependency rule is violated — fix the dependency, never the test.
