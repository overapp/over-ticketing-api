...
6. **Run** `dotnet test` (a container runtime must be running for the Aspire integration tests) and fix failures before finishing.
...
- **Integration test base:** inherit `BaseIntegrationTest(appHost)` with the `IntegrationTestAppHost` collection fixture; use `RegisterAndLoginAsync()` + `Authenticate(token)` for authenticated calls. Define private response DTO records inside the test class (see `TodosTests.TodoDto`).
